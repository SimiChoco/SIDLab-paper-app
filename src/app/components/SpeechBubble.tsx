...
const SpeechBubble: React.FC<SpeechBubbleProps> = ({
  ...
  const [tmpLikedNum, setTmpLikedNum] = useState(likedNum);
  ...
  return (
    <div
      className="relative flex bg-gray-200 rounded-lg p-3"
      style={{ display: comment ? "true" : "false", marginRight: "8px" }}
    >
      <p className="text-xs text-gray-800 line-clamp-2">{comment}</p>
      <div className="absolute top-1/2 -translate-y-1/2 left-[-8px] w-0 h-0 border-t-8 border-t-transparent border-b-8 border-b-transparent border-r-8 border-r-gray-200"></div>
        ...
        >
          ...
          >
            <path
              ...
              fill={liked ? "#FFDB5E" : "#D5D5D5"}
            />
          </svg>
          <span
            style={{ color: liked ? "#FFDB5E" : "#D5D5D5" }}
